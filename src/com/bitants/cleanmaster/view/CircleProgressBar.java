...
@SuppressLint("NewApi") public class CircleProgressBar extends View {
...
oval.left = progressStrokeWidth / 2+2; // 左上角x
oval.top = progressStrokeWidth / 2; // 左上角y
oval.right = width - progressStrokeWidth / 2; // 左下角x
oval.bottom = height - progressStrokeWidth / 2; // 右下角y
...
String text = "已用："+progress + "%";
int textHeight = height / 8;
paint.setTextSize(textHeight);
paint.setColor(Color.WHITE);
int textWidth = (int) paint.measureText(text, 0, text.length()+0);
paint.setStyle(Style.FILL);
canvas.drawText(text, width / 2 - textWidth / 2, height / 2 +textHeight/2+80, paint);
